...
const COLORS = ['bg-blue-400','bg-purple-400','bg-pink-400','bg-orange-400','bg-teal-400','bg-indigo-400','bg-rose-400','bg-amber-400','bg-cyan-400','bg-green-400']
const COLORS_LIGHT = ['bg-blue-100 text-blue-800','bg-purple-100 text-purple-800','bg-pink-100 text-pink-800','bg-orange-100 text-orange-800','bg-teal-100 text-teal-800','bg-indigo-100 text-indigo-800','bg-rose-100 text-rose-800','bg-amber-100 text-amber-800','bg-cyan-100 text-cyan-800','bg-green-100 text-green-800']
...
export default function CalendarView() {
  ...
  const cameraColorMap = useMemo(() => {
    const map = {}; let idx = 0
    rentals.forEach(r => { if (r.camera_id && !(r.camera_id in map)) { map[r.camera_id] = idx % COLORS.length; idx++ } })
    return map
  }, [rentals])

  const getRentalsOnDate = (ds) => rentals.filter(r => r.start_date <= ds && r.end_date >= ds)

  const getDayInfo = (d) => {
    if (!d) return { active: [] }
    const ds = dateStr(year, month, d)
    const active = getRentalsOnDate(ds).map(r => ({ ...r, isStart: r.start_date === ds, isEnd: r.end_date === ds, colorIdx: cameraColorMap[r.camera_id] ?? 0 }))
    return { ds, active }
  }
  ...
  const selectedRentals = selected ? getDayInfo(selected).active : []
  const monthStart = dateStr(year, month, 1)
  const monthEnd = dateStr(year, month, daysInMonth)
  const monthRentals = rentals.filter(r => r.start_date <= monthEnd && r.end_date >= monthStart)
      ...
      <div className="xl:col-span-3 bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <button onClick={() => setCurrent(new Date(year, month - 1))} className="p-2 hover:bg-gray-100 rounded-lg">
            <svg className="w-5 h-5 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" /></svg>
          </button>
          <div className="flex items-center gap-3">
            <h3 className="text-lg font-semibold text-gray-900">{MONTHS_TH[month]} {year + 543}</h3>
            <button onClick={() => { setCurrent(new Date()); setSelected(null) }} className="text-xs px-3 py-1 bg-brand-50 text-brand-600 hover:bg-brand-100 rounded-full font-medium">วันนี้</button>
          </div>
          <button onClick={() => setCurrent(new Date(year, month + 1))} className="p-2 hover:bg-gray-100 rounded-lg">
            <svg className="w-5 h-5 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" /></svg>
          </button>
        </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20"><div className="w-6 h-6 border-2 border-brand-500 border-t-transparent rounded-full animate-spin" /></div>
        ) : (
        ...
        {Object.keys(cameraColorMap).length > 0 && (
          <div className="px-5 py-3 border-t border-gray-100 flex flex-wrap gap-3">
            {rentals.filter((r, i, arr) => arr.findIndex(x => x.camera_id === r.camera_id) === i).map(r => (
      ...
      <div className="space-y-4">
        {selected && (
          <div className="bg-white rounded-2xl border border-gray-200 p-4">
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-semibold text-gray-900 text-sm">{selected} {MONTHS_TH[month]}</h4>
            ...
            {selectedRentals.length === 0 ? (
              <div className="text-center py-5">
                <div className="w-10 h-10 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-2">
                  <svg className="w-5 h-5 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
                </div>
                ...
                {selectedRentals.map(r => (
                  <div key={r.id} className={`rounded-xl p-3 ${COLORS_LIGHT[r.colorIdx]}`}>
                    <div className="flex items-start gap-2">
                      {r.camera?.image_url ? <img src={r.camera.image_url} className="w-8 h-8 rounded-lg object-cover flex-shrink-0" alt="" /> : <div className={`w-8 h-8 rounded-lg flex-shrink-0 ${COLORS[r.colorIdx]} opacity-40`} />}
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs opacity-75 mt-1">{r.start_date} → {r.end_date}</p>
                        {r.pickup_location && <p className="text-xs opacity-70 mt-0.5">📍 {r.pickup_location}</p>}
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-4">
          <h4 className="font-semibold text-gray-900 text-sm mb-3">
            รายการเดือนนี้
            {monthRentals.length > 0 && <span className="ml-2 text-xs bg-brand-100 text-brand-700 px-2 py-0.5 rounded-full">{monthRentals.length}</span>}
          </h4>
          {monthRentals.length === 0 ? (
            ...
            <div className="space-y-2">
              {monthRentals.map(r => (
                <div key={r.id} onClick={() => setSelected(parseInt(r.start_date.split('-')[2]))}
